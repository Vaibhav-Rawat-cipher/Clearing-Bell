...
import { AddressLink, TxLink } from '../ui/ChainState'
...
export function ProtocolReceipt({ open, onClose, onNavigate }: { open: boolean; onClose: () => void; onNavigate: (view: View) => void }) {
  ...
  return <Dialog open={open} onClose={onClose} titleId="receipt-title" className="protocol-dialog">
    ...
    {lastTxHash && <div className="receipt-txhash">
      <span className="receipt-txhash-label">Transaction hash</span>
      <TxLink hash={lastTxHash} label="View on HashScan ↗" />
      <AddressLink value={lastTxHash} transaction full />
    </div>}
